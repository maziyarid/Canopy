...
import type { CompetitorRow, GapRow, KeywordRow, QuotaState } from "./types";
...
export function mapCompetitors(raw: unknown, domain: string, projectId: string): CompetitorRow[] {
  const list = extractKeywordList(raw);
  return list.map((k, i) => ({
    id: String(k._id ?? i),
    projectId,
...
export function mapGaps(raw: unknown, projectId: string): GapRow[] {
  const data = raw as { results?: Array<{ domain?: string; items?: Record<string, unknown>[] }> };
  const rows: GapRow[] = [];
  for (const block of data.results ?? []) {
    for (const item of block.items ?? []) {
      rows.push({
        id: `${projectId}-${block.domain}-${item.keyword}`,
        projectId,
...
export function extractKeywordList(raw: unknown): Record<string, unknown>[] {
